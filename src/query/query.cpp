...
void load_index(string graph_name, string index_name, double omit_score_lb, int thre_make_seg, double seg_num_times, vector<vector<Itval>> &vsn)
{
    string index_file = "./index/" + graph_name + "_" + to_string((int)(omit_score_lb*100)) + "_" + to_string((int)(thre_make_seg)) + "_" + to_string((int)(seg_num_times*100)) + "_" + index_name + ".bin";
...
#ifdef _CheckInfo_
    for(ui i = 0; i < n; ++i) {
        cout<<i+1<<"-th vertex : "<<i<<endl;
        for(auto e : vsn[i]) {
            cout<<"\t\t"<<e.s_idx<<" -> "<<e.e_idx<<endl;
            cout<<"\t\t"<<e.min_score<<" -> "<<e.max_score<<endl;
            cout<<"\t\t"<<e.c<<endl;
            cout<<endl;
        }
    }
...
void vertex_reduction_by_vsn(ui* tmp_pstart, ui* tmp_edges, ui* tmp_degree, vector<vector<Itval>>& vsn)
{
    int thre = max_gc_size;
    if(thre < 1) return;
...
#ifdef _CheckInfo_
    cout<<"newid mapping infomration:"<<endl;
    for(ui i = 0; i < n; ++i) {
        if(del[i]==0) cout<<"\t"<<i<<"  ->  "<<newid_mapping[i]<<endl;
    }
#endif
...
#ifdef _CostlyDebug_
    for(ui i = 0; i < n; ++i) {
        assert(tmp_degree[i] == (tmp_pstart[i+1] - tmp_pstart[i]) );
        assert(tmp_degree[i] < n);
    }
#endif
#ifdef _CheckInfo_
    cout<<"tmp graph infomration:"<<endl;
    for(ui i = 0; i < n; ++i) {
...
void extends_res_by_index(vector<vector<Itval>>& vsn)
{
    assert(!res.empty() && res.size()==1 && res[0].size() == max_gc_size );
    ...
    for(auto u : res[0]) {
        ...
        vector<pair<int, ui>> SimNei;
        unordered_set<ui> StructNei;
        for(ui i = pstart[u]; i < pstart[u+1]; ++i) StructNei.insert(edges[i]);
        vector<Itval> & u_seg_list = vsn[u];


        for(auto &seg : u_seg_list) if(seg.max_score >= epsi) {
            for(ui v = seg.s_idx; v <= seg.e_idx; ++v) {
                if(v==u)continue;
                auto p = js(u, v);
                if(p.first >= epsi && StructNei.find(v)==StructNei.end() ) SimNei.push_back(make_pair(p.second, v));
            }
        }
        sort( SimNei.begin(), SimNei.end());
        sort(SimNei.begin(), SimNei.end(), [](const pair<int, ui>& a, const pair<int, ui>& b) { return a.first > b.first; } );

        unordered_map<ui, unordered_set<ui> > hash_adj;
        for(auto x : res[0]) for(ui i = pstart[x]; i < pstart[x+1]; ++i) hash_adj[x].insert(edges[i]);
        for(auto x : SimNei) for(ui i = pstart[x.second]; i < pstart[x.second+1]; ++i) hash_adj[x.second].insert(edges[i]);


        vector<ui> expanded_res;
        expanded_res = res[0];
        for(auto vp : SimNei) {
            ...
            bool f = true;
            for(auto w : expanded_res) {
                assert(w!=v && hash_adj.find(w) != hash_adj.end());
                if( hash_adj[w].find(v)==hash_adj[w].end() && js(w,v).first < epsi) {
                    f = false; break;
                }
            }
            if(f) expanded_res.push_back(v);
        }
        if(expanded_res.size() > tmp_res.size()) tmp_res = expanded_res;
    }

    if(tmp_res.size() > res[0].size()) {
        res[0] = tmp_res;
        max_gc_size = (int)tmp_res.size();
    }
}
...
void maximum_generalized_clique_computation_by_index(vector<vector<Itval>> &vsn)
{
    assert(!vsn.empty());
    Timer t;
    Timer tt;
    ...
    extends_res_by_index(vsn);
    cout<<"exp heu solu size: "<<max_gc_size<<endl;

    t_heu += (double)tt.elapsed() / CLOCKS_PER_SEC;


    ui * tmp_pstart; tmp_pstart = new ui[n+1];
    ui * tmp_edges; tmp_edges = new ui[m];
    ui * tmp_degree; tmp_degree = new ui[n];
    memcpy(tmp_pstart, pstart, sizeof(ui)*(n+1));
    memcpy(tmp_edges, edges, sizeof(ui)*m);
    memcpy(tmp_degree, degree, sizeof(ui)*n);

    tt.restart();
    vertex_reduction_by_vsn(tmp_pstart, tmp_edges, tmp_degree, vsn);
    t_vr += (double)tt.elapsed() / CLOCKS_PER_SEC;

    cout<<"after vr, |V| = "<<n<<", |E| = "<<m/2<<endl;
    ...
    tt.restart();
    compute_degeneracy_ordering_of_tmp_graph(peels,vis,core,rid,tmp_pstart, tmp_edges, tmp_degree);
    t_degen += (double)tt.elapsed() / CLOCKS_PER_SEC;
    ...
    for(int i = n-1; i >=0; --i) {
...
#endif
        vector<ui> StructNei;
        for(ui j = tmp_pstart[u]; j < tmp_pstart[u+1]; ++j) if(rid[tmp_edges[j]] > rid[u]) {
            vis[tmp_edges[j]] = 1;
            StructNei.push_back(tmp_edges[j]);
        }
        vector<Itval> & u_seg_list = vsn[oid_mapping[u]];
#ifdef _CheckInfo_
        ...
        for(ui j = i+1; j < n; ++j) if(vis[peels[j]]==0) {
            ...
            for(auto e : u_seg_list) {
                if(e.s_idx<=oid_mapping[peels[j]] && oid_mapping[peels[j]]<=e.e_idx && e.max_score >= epsi) {

                    if(js_decision(oid_mapping[u], oid_mapping[peels[j]])) {
                        progress_sim_nei[u].push_back(peels[j]);
                        progress_sim_nei[peels[j]].push_back(u);
                    }
                    break;
                }
            }
        ...
        sort(progress_sim_nei[u].begin(),progress_sim_nei[u].end());
#ifdef _CheckInfo_
        cout<<"\t\tStruct Nei: ";
        for(auto e : StructNei) cout<<e<<","; cout<<endl;
        cout<<"\t\tSimila Nei: ";
        for(auto e : progress_sim_nei[u]) cout<<e<<","; cout<<endl;
#endif
        if(1 + StructNei.size() + progress_sim_nei[u].size() <= max_gc_size) {
            for(auto e : StructNei) vis[e]=0;
            continue;
        }
        ...
        StructNei.push_back(u);
        vis[u]=1;
        for(auto e : progress_sim_nei[u]) {
            StructNei.push_back(e);
            vis[e]=1;
        }
        build_egosg_and_vr(tmp_pstart, tmp_edges, tmp_degree, StructNei, vis, sg_vs, sg_ve, sg_e, sg_deg, progress_sim_nei);
        t_construct_subgraph += (double)tt.elapsed() / CLOCKS_PER_SEC;
        ...
        tt.restart();
        ui md = StructNei.size();
        Matrix = new char*[md];
        for(int j = 0; j < md; j++) Matrix[j] = new char[md];
        auto it = find(StructNei.begin(), StructNei.end(), u);
        assert(it != StructNei.end());
        if(it != StructNei.end()) StructNei.erase(it);
        build_matrix(u, StructNei, vis, progress_sim_nei,tmp_pstart, tmp_edges, tmp_degree);
        t_build_matrix += (double)tt.elapsed() / CLOCKS_PER_SEC;
    ...
    cout << "Time: "<<total_time<<"s"<<endl;
    cout << "\theu solution t = " << t_heu << " s,  " <<(double)t_heu/total_time*100<<" %"<< endl;
    cout << "\tindex-based vr = " << t_vr << " s,  " <<(double)t_vr/total_time*100<<" %"<< endl;
    cout << "\tdegeneracy t   = " << t_degen << " s,  " <<(double)t_degen/total_time*100<<" %"<< endl;
    cout << "\tsim nei t      = " << t_simnei << " s,  " <<(double)t_simnei/total_time*100<<" %"<< endl;
    cout << "\tconstruct sg t = " << t_construct_subgraph << " s,  " <<(double)t_construct_subgraph/total_time*100<<" %"<< endl;
    cout << "\tbuild matrix t = " << t_build_matrix << " s,  " <<(double)t_build_matrix/total_time*100<<" %"<< endl;
...
static void print_usage() {
    cout << "Usage:\n"
         ...
         << "  global MGC-VSN <omit_score_lb> <segment_threshold> <segment_ratio>\n\n"
         << "Vertex query:\n"
...
int main(int argc, const char * argv[]) {
    ...
    if(mode == "global") {
        if(method == "MGC-Mat") {
            ...
            const double omit_score_lb = stod(argv[7]);
            const int segment_threshold = stoi(argv[8]);
            const double segment_ratio = stod(argv[9]);

            load_index(graph_name, "VSN",
                       omit_score_lb, segment_threshold, segment_ratio, vsn);
            maximum_generalized_clique_computation_by_index(vsn);
        }
        else {
