...
Source code for maximum generalized clique (MGC) computation and the indexes used by the indexed algorithms.

The repository contains two executables:

- `build_index`: builds DB-Index, BC-Index, or VSN-Index;
...
`DB` and `DB-Batch` construct the same type of **DB-Index**. `DB-Batch` is the memory-bounded implementation used when the intermediate similarity graph is too large to construct in memory at once.
...
```bash
./bin/build_index DATA_DIR GRAPH VSN \
...
```bash
./bin/query DATA_DIR GRAPH EPS SIZE global MGC-VSN \
    OMIT_SCORE_LB SEGMENT_THRESHOLD SEGMENT_RATIO
```

## Vertex MGC query

The `vertex` mode returns an MGC containing a specified query vertex `Q`.
...
| `MGC-VSN` | Uses the VSN index baseline |
